...
(function($) {
    ...
    const StaffPanel = {
        ...
        /**
         * Handle sub-status checkbox change
         */
        handleSubStatusChange: function(e) {
            const $checkbox = $(e.target);
            const orderId = $checkbox.data('order-id');
            const subStatusKey = $checkbox.data('substatus-key');
            ...
            this.updateSubStatus(orderId, subStatusKey, completed, $checkbox);
        },

        /**
         * Update sub-status via REST API
         */
        updateSubStatus: function(orderId, subStatusKey, completed, $checkbox) {
            const self = this;
            const restUrl = buildRestUrl(tabeshStaffVars.restUrl, 'update-substatus');
            
            $.ajax({
                url: restUrl,
                method: 'POST',
                headers: {
                    'X-WP-Nonce': tabeshStaffVars.nonce
                },
                contentType: 'application/json',
                data: JSON.stringify({
                    order_id: orderId,
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        if (completed) {
                            ...
                            const staffName = response.staff_name || '';
                            
                            // Remove existing completion info if any
                            $item.find('.substatus-completed-info').remove();
                            
                            // Add new completion info
                            $item.find('.substatus-info').append(
                        ...
                        // Update progress bar
                        const $section = $item.closest('.printing-substatus-section');
                        const completion = response.completion || 0;
                        $section.find('.substatus-progress').text(completion + '%');
                        $section.find('.substatus-progress-fill').css('width', completion + '%');
                        
                        // Check if status was auto-completed
                        if (response.status_changed && response.new_status === 'ready') {
                            self.showToast(
                                'تمام مراحل چاپ تکمیل شد! وضعیت به "آماده تحویل" تغییر یافت.',
                                'success'
                            );
                            
                            // Update the status stepper
                            const $card = $checkbox.closest('.tabesh-staff-order-card');
                            $card.find('.stepper-step[data-status="processing"]').removeClass('active').addClass('completed');
                            $card.find('.stepper-step[data-status="ready"]').addClass('active');
                            
                            // Update status badge
                            $card.find('.status-badge').removeClass('status-processing').addClass('status-ready').text('آماده');
                            $card.attr('data-status', 'ready');
                            
                            // Hide sub-status section (no longer in processing)
                            setTimeout(function() {
    ...
    $(document).ready(function() {
        if ($('.tabesh-staff-panel').length > 0) {
            ...
            // Bind sub-status checkbox handler
            $(document).on('change', '.substatus-checkbox', function(e) {
                StaffPanel.handleSubStatusChange.call(StaffPanel, e);
            });
        }
    });
