...
class Tabesh_Archive {
    ...
    public function register_rest_routes() {
        register_rest_route(TABESH_REST_NAMESPACE, '/archive/archived', array(
            'methods'             => 'GET',
            'callback'            => array($this, 'rest_get_archived_orders'),
            'permission_callback' => array($this, 'check_admin_permission'),
        ));

        register_rest_route(TABESH_REST_NAMESPACE, '/archive/cancelled', array(
            'methods'             => 'GET',
            'callback'            => array($this, 'rest_get_cancelled_orders'),
            'permission_callback' => array($this, 'check_admin_permission'),
        ));

        register_rest_route(TABESH_REST_NAMESPACE, '/archive/reorder', array(
            'methods'             => 'POST',
    ...
    private function get_orders_by_archive_status($archived, $statuses, $page = 1, $per_page = 20, $search = '') {
        ...
        // Get total count
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $where_sql is built safely above
    ...
    public function reorder($source_order_id, $new_user_id) {
        ...
        $source_order = $wpdb->get_row($wpdb->prepare(
    ...
    public function archive_existing_orders() {
        ...
        $result = $wpdb->query($wpdb->prepare(
            "UPDATE $table SET archived = 1, archived_at = %s 
             WHERE archived = 0 AND status IN ('delivered', 'completed', 'cancelled')",
            current_time('mysql')
        ));
