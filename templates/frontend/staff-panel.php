                ...
                ?>
                    <div class="tabesh-staff-order-card" 
                        ...
                        <div class="order-card-body">
                            ...
                            <?php 
                            // Sub-Status Tracking for Printing Process
                            ...
                            ?>
                                <div class="printing-substatus-section" data-order-id="<?php echo esc_attr($order->id); ?>">
                                    <div class="section-header">
                                        ...
                                        <h4 class="section-title"><?php _e('مراحل جزئی چاپ', 'tabesh'); ?></h4>
                                        <span class="substatus-progress"><?php echo esc_html($completion); ?>%</span>
                                    </div>
                                    
                                    <div class="substatus-progress-bar">
                                        <div class="substatus-progress-fill" style="width: <?php echo esc_attr($completion); ?>%;"></div>
                                    </div>
                                    
                                    <div class="substatus-checklist">
                                        <?php 
                                        foreach ($sub_status_definitions as $key => $definition):
                                            ...
                                            // Check if this sub-status exists in data
                                            if (!isset($sub_status_data[$key])) {
                                                continue;
                                            }
                                            
                                            $is_completed = $sub_status_data[$key]['completed'] ?? false;
                                            $completed_at = $sub_status_data[$key]['completed_at'] ?? null;
                                            $completed_by = $sub_status_data[$key]['completed_by'] ?? null;
                                            
                                            // Get order field value for display
                                            $field_value = '';
                                            if (isset($definition['order_field'])) {
                                                $field_name = $definition['order_field'];
                                                if ($field_name === 'extras') {
                                                    $field_value = !empty($extras) ? implode('، ', $extras) : '';
                                                } else {
                                                    $field_value = $order->$field_name ?? '';
                                                    // Add extra field if defined
                                                    if (isset($definition['order_field_extra'])) {
                                                        $extra_field = $definition['order_field_extra'];
                                                        $extra_value = $order->$extra_field ?? '';
                                                        if (!empty($extra_value)) {
                                                            $field_value .= ' - ' . $extra_value . 'g';
                                                        }
                                                    } elseif (in_array($field_name, array('cover_paper_weight', 'paper_weight'))) {
                                        ...
                                        ?>
                                            <div class="substatus-item <?php echo $is_completed ? 'completed' : ''; ?>" 
                                                 data-substatus-key="<?php echo esc_attr($key); ?>">
                                                <label class="substatus-checkbox-label">
                                                    <input type="checkbox" 
                                                           class="substatus-checkbox" 
                                                           <?php checked($is_completed); ?>
                                                           data-order-id="<?php echo esc_attr($order->id); ?>"
                                                           data-substatus-key="<?php echo esc_attr($key); ?>">
                                                    <span class="substatus-checkmark"></span>
                                                    <div class="substatus-info">
                                                        <span class="substatus-label"><?php echo esc_html($definition['label']); ?></span>
                                                        <?php if (!empty($field_value)): ?>
                                                            <span class="substatus-details">
                                                                <?php echo esc_html($definition['order_field_label']); ?>: 
                                                                <strong><?php echo esc_html($field_value); ?></strong>
                                                            </span>
                                                        <?php endif; ?>
                                                        <?php if ($is_completed && $completed_at): ?>
                                                            <span class="substatus-completed-info">
                                                                ✓ <?php echo esc_html(date_i18n('Y/m/d H:i', strtotime($completed_at))); ?>
                                                                <?php 
                                                                if ($completed_by) {
                                                                    $staff_user = get_userdata($completed_by);
                                                                    if ($staff_user) {
                                                                        echo ' - ' . esc_html($staff_user->display_name);
                                                                    }
                                                                }
                                                                ?>
                                                            </span>
                                    ...
                                    <div class="substatus-note">
                                        <span class="note-icon">ℹ️</span>
                                        <span class="note-text"><?php _e('با تکمیل تمام مراحل، وضعیت سفارش به صورت خودکار به "آماده تحویل" تغییر می‌کند', 'tabesh'); ?></span>
                                    </div>
                                </div>
                            <?php endif; ?>
